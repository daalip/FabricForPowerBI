...
Navigate to the "Fabric Workshop xxx" workspace and click the "+ New item" button from the ribbon. Scroll down to the "Store data" section and select the "Eventhouse" tile. Alternatively, choosed the "Real-Time Intelligence" persona from the persona switcher in the bottom left corner of the window and select the "Eventhouse" tile in the "Recommended items to create" section.
...
Add some sample data. Click on the "Get data" dropdown tile, locate and select the "Sample" tile. and select "Automotive operations analytics". This is a truncated database of New York taxi operations over a two year period, and the largest provided sample data set. This will take a few moments to load.

![](assets/04_Realtime_eventhouse_sample.png)

Select the "Automotive" table, then select "Schema insights" to view the schema.

Select the "MyKusto" database in the "KQL databases" section. Click the "Explore your data" button in the upper right hand corner. A window will open with some sample queries.
...
Open the "MyKusto" KQL database (if you choose the MyKusto Eventhouse by accident, just click on the "MyKusto" database from the Eventhouse). Locate the OneLake section on the right side of your screen.

Set the value to "On", a window will pop-up, make sure the "Apply to existing tables" box is checked and click "Turn on".
...
Click the "Create" button. The "Automotive" table appears under the "Tables" node. It will likely appear empty, as the process of mirroring the KQL data into the lakehouse can take some time, but you will eventually see the KQL data displayed as a delta table named "Automotive" in the lakehouse. You may need to come back to this spot. 
...
Navigate to the "Fabric Workshop xxx" workspace and click the "+ New item" button from the ribbon. Scroll down to the "Get data" section and select the "Eventstream" tile.
...
Name the source "Rental_Bikes" and select "Bicycles" from the dropdown menu. Click the "Add" button.
...
Change from "Lakehouse" to "SQL analytics endpoint" with the button in the upper right of the ribbon. Navigate to the "Reporting" tab and select "Manage default semantic model" from the ribbon. Locate the "BikesLive" table inside of the "Tables" folder and ensure that the table is selected.

Click the "Confirm" button.

Navigate to the "Model layouts" tab in the "Explorer" section.

Locate the "BikesLive" table and select the elipsis, then select "New Measure". Add the following measure:

```PlainText
Latest (UTC) = MAX(BikesLive[EventProcessedUtcTime])
```

Navigate to the "Home" tab and select the "Metadata sync" button. Navigate back to the "BikesLive" table in the "Model layouts" tab and click on the "Latest (UTC)" measure.
